...
import (
	...
	"catalyst/src/interface/rest/handlers"
	"catalyst/src/interface/rest/response"
	"catalyst/src/interface/rest/route"
...
func makeRoute(
	...
	bh := brandHandler.NewBrandHandler(respClient, useCases.BrandUseCase)
	ph := productHandler.NewProductHandler(respClient, useCases.ProductUseCase)
	oh := orderHandler.NewOrderHandler(respClient, useCases.OrderUseCase)
	r.Route("/api", func(r chi.Router) {
		r.Mount("/check", route.HealthRouter(hh))
		r.Mount("/brand", route.BrandAppRouter(bh))
		r.Mount("/product", route.ProductAppRouter(ph))
		r.Mount("/order", route.OrderAppRouter(oh))
